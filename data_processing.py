...
my_table3_join = my_table3.join(my_table4, 'team')
my_table3_team = my_table4.select("team")
my_table3_filtered = my_table3.filter(lambda x: "ia" in str(x['team'])).filter(lambda x: int(x['minutes']) < 200).filter(lambda x: int(x['passes']) > 100)
print(my_table3_filtered)
my_table4_filtered_low = my_table4.filter(lambda x: int(x['ranking']) < 10 )
my_table4_filtered_high = my_table4.filter(lambda x: int(x['ranking']) >= 10 )
print("Low rank",my_table4_filtered_low.aggregate(lambda x: sum(x)/len(x), 'games'),"High rank",my_table4_filtered_high.aggregate(lambda x: sum(x)/len(x), 'games'))
...
print("Forwards",my_table_forward.aggregate(lambda x: sum(x)/len(x), 'passes'),"Midfielders",my_table_mid.aggregate(lambda x: sum(x)/len(x), 'passes'))

table5 = Table ("titanic",titanic)
my_DB.insert(table5)
my_table5=my_DB.search("titanic")
first_class=my_table5.filter(lambda x: x["class"]=="1")
third_class=my_table5.filter(lambda x: x["class"]=="3")
print("First class",first_class.aggregate(lambda x: sum(x)/len(x), 'fare'),"Third class",third_class.aggregate(lambda x: sum(x)/len(x), 'fare'))
